...
<div class="row justify-content-center p-6">
    <div class="col-md-12">
        <!-- DATA TABLE -->
        <h3 class="title-5 m-b-35">Current Deliveries</h3>
            ...
            <table class="table table-data2">
                <thead>
                    <tr>
                        ...
                        <th>Quantity</th>
                        <th>Fee</th>
                        <th>Date Offer Accepted</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {{ $current_deliveries_rows }}
                </tbody>
            </table>
        </div>
        <!-- END DATA TABLE -->
...
<script>
    ...
    // Change value of hidden input in "make delivery offer" so that it corresponds 
    // to the current value of the "fee" input visible to the user.
    $(".makeDeliveryOfferFeeInput").on("input", function() {
        const fee = $(this).val();
        $(this)
            .closest("tr")
            .find(".makeDeliveryOfferFeeInputHidden")
            .val(fee);
    });

    $(".makeDeliveryOfferButton").click(function(e) {
        const hiddenFeeInputValue = $(this)
            .closest("td")
            .find(".makeDeliveryOfferFeeInputHidden")
            .val();
        if (!hiddenFeeInputValue) {
            e.preventDefault();
            showFixedPositionAlert("danger", "Please enter a fee first!");
        }
    });
